...
export const getPostList = (type, sendData, fn) => {
    return (dispatch) => {
        let _url = type === 'init' ? '/post/list' : '/post/search'
        axiosAjax('post', _url, !sendData ? {} : sendData, function (res) {
            if (res.status === 200) {
                const actionData = res.data
                dispatch(addPostData({'list': actionData.datas}))
                dispatch(setPageData({'totalCount': actionData.totalCount, 'pageSize': actionData.pageSize, 'currPage': actionData.currentIndex}))
                if (fn) {
                    fn(actionData)
                }
            } else {
...
export const getPostItemInfo = (sendData, fn) => {
    return (dispatch) => {
        axiosAjax('post', '/post/detail', {...sendData, 'appId': $.cookie('gameId')}, function (res) {
            if (res.status === 200) {
                const actionData = res.data
